...
<div class="container">
    ...
    <table id="table1" class="table">
        ...
        <tbody>
            @if ($users->isEmpty())
            <tr>
                <td colspan="8">Okuyucu bulunamadı.</td>
            </tr>
            @endif
            ...
            <tr>
                ...
                <td><a href="{{ route('readers.rentals', $user->id) }}" class="btn btn-sm btn-primary">Ödünç Almaları Görüntüle</a></td>
                <td>
                    @auth
                    @if ($isAdmin)
                    <form action="{{ route('readers.promotelibrarian', $user->id) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn-danger btn-sm">Kütüphaneci Yap</button>
                    </form>
                    @endif
                    @endauth
                </td>
